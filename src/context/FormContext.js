...
const toggleCheckbox = (name, isChecked) => {
    ...
    const handleChkBoxChange = (event) => {
        ...
        toggleCheckbox(name, isChecked );
       const newTotalValue = isChecked ? totalValue + value : totalValue - value;
...
const {
    ...
    return (
        <FormContext.Provider value={{ title, page, setPage, data, setData, canSubmit, handleChange, 
             checkboxes, handleChkBoxChange, totalValue, disablePrev, disableNext, prevHide, nextHide, submitHide }}>
            {children}
